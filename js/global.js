...
$(function() {
	...
	/*cart heart*/
	$('.open-cart-text').on('mouseover', function(e){
		clearTimeout(closecartTimeout);
		
		if(!$('.cart-text.popup').hasClass('active')){
			closePopups();
			if($(this).offset().left>winW*0.5){
				$('.cart-text.popup').addClass('active cart-right').css({'left':'auto', 'right':winW - $(this).offset().left-$(this).outerWidth()*0.5-47, 'top':$(this).offset().top-winScr+15, 'opacity':'0'}).stop().animate({'opacity':'1'}, 300);			
			}
			else{
				$('.cart-text.popup').addClass('active cart-left').css({'right':'auto', 'left':$(this).offset().left, 'top':$(this).offset().top-winScr+15, 'opacity':'0'}).stop().animate({'opacity':'1'}, 300);			
			}
		}
		//if($(this).offset().left<100) $('.cart-text.popup').addClass('left-align');
		//else if($(this).hasClass('header-functionality-entry') && $(this).closest('header.type-3').length) $('.cart-text.popup').addClass('fixed-header-left');
		//else $('.cart-text.popup').removeClass('left-align');
	});
	
	$('.open-cart-text').on('mouseleave', function(){
